...
def kerkesa(request):
    if request.method == 'POST':
        ...
        message = 'Kerkesa juaj per nje llogari mesuesi u pranua! Tani ju mund te ktheheni tek ShkollaSeArdhmes dhe te ngarkoni kurse dhe leksione, pune te mbare!'
        send_mail(
            'ShkollaSeArdhmes, kerkesa u pranua.',
            message,
            'ShkollaSeArdhmes@no-reply.com',
            [email],
            fail_silently=False,
        )
        send_mail(
...
def stafi(request):
    return render(request, 'stafi.html')


def sekretari(request):
    return render(request, 'psikologia_index.html')
